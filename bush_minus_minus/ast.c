...
/*** TO BE DONE START ***/
	...
	if (chdir(impl->pathname) < 0)
		printf("No such file or directory\n");

    struct var_table *const vars = sh_get_var_table(sh);

    char *cwd = getcwd(NULL, 0);
    vt_set_value(vars, "OLDPWD", (char *const) vt_lookup(vars, PWD));
    vt_set_value(vars, PWD, cwd);
    free(cwd);
/*** TO BE DONE END ***/
...
char *find_in_path(const char *path, const char *name)
{
	/* Se name contiene degli '/', restituirne una copia con strdup().
	 V* Altrimenti, fare il parsing del path con strchr, per cercare il comando in tutte le directory.
...
/*** TO BE DONE START ***/
  if(name == NULL)    						//se name e' NULL Error
      fail_errno("NULL name error");

  if(strchr(name, PATH_SEPARATOR) != NULL)   //controllo se nella stringa e' presente '/'
     return my_strdup(name);                  //copia con strdup
      ...
      long int allocated = -1, len = 0;

      size_t nameLen = strlen(name);
      size_t pathLen = strlen(path);

      for(int i = 0; i <= pathLen; i += len + 1) {
          start = path + i;

          if((end = strchr(start, DIR_SEPARATOR)) == NULL)
              end = path + pathLen;

          len = end - start;
          size_t totLen = len + nameLen + 2;

          if(allocated < len) {
              currPath = my_realloc(currPath, totLen);
              allocated = len;
          }

          memcpy(currPath, start, len);

          sprintf(currPath + len, "%c%s", PATH_SEPARATOR, name);

          if(!access(currPath, X_OK)) {
              char *tmp = my_malloc(totLen);
              memcpy(tmp, currPath, totLen);
              free(currPath);
              return tmp;
          }
      }
...
/*** TO BE DONE END ***/
	if (pid == 0) {
		 ...
		 * la syscall fallisce. */
/*** TO BE DONE START ***/
        if(execve(executable, impl->args, vt_to_envp(sh_get_var_table(sh))) < 0) {
            fail_errno("execute error");
        }
/*** TO BE DONE END ***/
	}
	cleanup_and_return:
