...
/*** TO BE DONE START ***/
	...
	if ((tcp_socket = socket(server_addrinfo->ai_family, server_addrinfo->ai_socktype, server_addrinfo->ai_protocol)) < 0)
...
/*** TO BE DONE START ***/
	...
	if((send(tcp_socket, request, len, 0)) != len)
