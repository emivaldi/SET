...
/*** TO BE DONE START ***/
 	...
 	double mSec = (last->tv_nsec / 1000000.0) - (previous->tv_nsec / 1000000.0);
	double mSec1 = (last->tv_sec * 1000.0) - (previous->tv_sec * 1000.0);
...
/*** TO BE DONE START ***/
	...
	int x;
	for(x = 0; x < N_HISTOGRAM_ITEMS; x++) {
		printf("%f ", h_min += h_incr);
		printf("%d\n", histogram[x]);
	}
